...
http.createServer((req, res) => {
  const filePath = path.join(DIR, req.url === '/' ? 'fulcrum-chiropractic.html' : req.url.split('?')[0]);
...
}).listen(PORT, () => console.log(`Fulcrum Chiropractic server running on http://localhost:${PORT}`));
