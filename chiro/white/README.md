# SGEN Chiropractic (Light) — Local SEO + AEO Landing Page

**Light/white-theme** editable static bundle of the SGEN Local SEO + AEO landing page,
retargeted from **dentistry to chiropractic**. Counterpart of the
`SGEN-SEO-Chiro_Black - Optimized` bundle — identical chiropractic content, stats and
website samples, rendered on SGEN's light theme. Static HTML/CSS/JS — no build step.

Source reference: `SGEN-SEO-AEO-Dental_White_Optimized_V4` (locked master — V4, never V3).
...
Critical path (`index.html` + `chrome.css`, gzipped): **234.1 KB → 190.7 KB**, 43 KB less.
...
- jQuery repointed from the remote copy to the local 89,501-byte file and deferred — safe because
  the page contains 0 `jQuery` tokens and both `# SGEN Chiropractic (Light) — Local SEO + AEO Landing Page
...
| File | Purpose |
|------|---------|
| `index.html` | The chiropractic Local SEO + AEO landing page — **light theme** |
...
```
cd "SGEN-SEO-Chiro_White - Optimized"
...
**201 audited operations / 260 substitutions** applied to `index.html`. Every replacement declared
...
- To adjust the light theme, edit the `<style id="light-theme">` block at the end of `index.html`.
