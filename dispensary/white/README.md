# SGEN — Cannabis Dispensary Local SEO + AEO · White (light)

Retarget of `SGEN-SEO-AEO-Dental_White_Optimized_V4` from **dental** to **cannabis dispensaries**.
...
declared how many occurrences it expected and refused to write anything on a mismatch.

## How white was derived

Built from `SGEN-SEO-AEO-Dental_White_Optimized_V4` and given the finished black bundle's asset
tree — **never re-themed from black**. Only 2 of 227 files differ between the two dental
references, but their `index.html` differ by 265 line-records and the delta cuts both ways, so
starting from the white source makes the theme correct by construction.

Before running the shared replacement list, all **14 content anchors were confirmed present in the
white source at counts identical to black**, so the same list could not mis-target. The rewrite
produced 315 substitutions.

One black-only difference surfaced and is handled: the layout-fix CSS comment mentioning
"find a local dentist" exists only in the black file, so that replacement is optional.

Theme survival verified: `<style id="light-theme">` present, `sgen-logo.png` ×10 and
`sgen-logo-3.webp` ×3 — matching the white reference exactly (the dark logo refs are intentional;
they sit on the mobile-menu and apply-overlay dark surfaces) — and at runtime `body` computes to
`rgb(255, 255, 255)`.
...
| Metric | Reference (dental V4 white) | This build |
|---|---|---|
| Files | 227 | 305 |
...
- **0 NEW console errors and 0 NEW failed requests** vs the white reference's own baseline. The
  reference logs 10–14 errors / 6–8 failed requests (file:// CORS on fonts + the `sg-collect`
  beacon); this build logs 8–12, i.e. equal or fewer, with an identical error set.
...
- Each mockup renders standalone: 0 broken images, 0 console errors, 0 failed requests, and the
  ...
  pruning was safe
- Visible-text sweep: **4 hits remain**, each justified below
...
4. `belts-and-braces` — an idiom in a CSS comment about the atmosphere painters, not orthodontics.
   This one is white-only (the black file's comment differs).
